...
/obj/machinery/computer/ftl_navigation/ui_interact(mob/user)
	. = ..()
	if(mode != "Spoolup" && SSftl_navigation.ftl_state == FTL_SPOOLUP) mode = "Spoolup" //Fuck popup uis
	if(mode != "Transit" && SSftl_navigation.ftl_state == FTL_JUMPING) mode = "Transit"
	if((mode == "Spoolup" || mode == "Transit") && SSftl_navigation.ftl_state == FTL_IDLE) mode = "Options"
	...
	switch(mode)
		if("Options")
			...
			if(SSftl_navigation.current_sector.planet) dat += "\A [SSftl_navigation.current_sector.planet] is within range of the FOB.<br>"
			if(SSftl_navigation.current_sector.station) dat += "\A [SSftl_navigation.current_sector.station] is within trading range.<br>"
			dat += "<hr>Adjacent systems:<hr>"
			for(var/datum/sector/S in SSftl_navigation.current_sector.connected_sectors)
		...
		if("Details")
			...
			//long range results
			if(sector_details.planet) dat += "Star charts indicate a planet will be in this region.<br>"
			if(sector_details.station) dat += "The Galactic Trading Association lists an operational station open for trading here.<br>"
			//Name only for this sectors connections
			dat += "<hr>Connected sectors:<br>"
			for(var/datum/sector/S in sector_details.connected_sectors)
				dat += "-[S.name]<br>"
